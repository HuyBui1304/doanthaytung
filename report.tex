...
Kết quả được trình bày thông qua ma trận trực quan gồm 9 biểu đồ, tương
ứng với 9 tổ hợp siêu tham số khác nhau. Mỗi điểm trên biểu đồ đại diện
cho một mẫu dữ liệu cảm biến, được tô màu theo nhãn Activity.
...
Tiếp theo chúng em tiến hành tổng hợp và so sánh hiệu suất của các mô
hình dựa trên các chỉ số đánh giá quan trọng như Accuracy, Macro F1 và
Weighted F1.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{model\_summary }\OtherTok{\textless{}{-}} \FunctionTok{data.frame}\NormalTok{(}
  \AttributeTok{Model =} \FunctionTok{c}\NormalTok{(}\StringTok{"Logistic Regression"}\NormalTok{, }\StringTok{"SVM (Linear)"}\NormalTok{, }\StringTok{"Decision Tree"}\NormalTok{, }\StringTok{"Random Forest"}\NormalTok{, }\StringTok{"KNN"}\NormalTok{),}
  \AttributeTok{Accuracy =} \FunctionTok{c}\NormalTok{(}
\NormalTok{    results\_log}\SpecialCharTok{$}\NormalTok{test\_accuracy,}
\NormalTok{    results\_svc}\SpecialCharTok{$}\NormalTok{test\_accuracy,}
\NormalTok{    results\_dt}\SpecialCharTok{$}\NormalTok{test\_accuracy,}
\NormalTok{    results\_rf}\SpecialCharTok{$}\NormalTok{test\_accuracy,}
\NormalTok{    results\_knn}\SpecialCharTok{$}\NormalTok{test\_accuracy}
\NormalTok{  ),}
  \AttributeTok{Macro\_F1 =} \FunctionTok{c}\NormalTok{(}
\NormalTok{    results\_log}\SpecialCharTok{$}\NormalTok{macro\_metrics[}\StringTok{"F1"}\NormalTok{],}
\NormalTok{    results\_svc}\SpecialCharTok{$}\NormalTok{macro\_metrics[}\StringTok{"F1"}\NormalTok{],}
\NormalTok{    results\_dt}\SpecialCharTok{$}\NormalTok{macro\_metrics[}\StringTok{"F1"}\NormalTok{],}
\NormalTok{    results\_rf}\SpecialCharTok{$}\NormalTok{macro\_metrics[}\StringTok{"F1"}\NormalTok{],}
\NormalTok{    results\_knn}\SpecialCharTok{$}\NormalTok{macro\_metrics[}\StringTok{"F1"}\NormalTok{]}
\NormalTok{  ),}
  \AttributeTok{Weighted\_F1 =} \FunctionTok{c}\NormalTok{(}
\NormalTok{    results\_log}\SpecialCharTok{$}\NormalTok{weighted\_metrics[}\StringTok{"F1"}\NormalTok{],}
\NormalTok{    results\_svc}\SpecialCharTok{$}\NormalTok{weighted\_metrics[}\StringTok{"F1"}\NormalTok{],}
\NormalTok{    results\_dt}\SpecialCharTok{$}\NormalTok{weighted\_metrics[}\StringTok{"F1"}\NormalTok{],}
\NormalTok{    results\_rf}\SpecialCharTok{$}\NormalTok{weighted\_metrics[}\StringTok{"F1"}\NormalTok{],}
\NormalTok{    results\_knn}\SpecialCharTok{$}\NormalTok{weighted\_metrics[}\StringTok{"F1"}\NormalTok{]}
\NormalTok{  )}
\NormalTok{)}
...
## 1 Logistic Regression 0.9487615 0.9482859   0.9486186
## 2        SVM (Linear) 0.9606379 0.9603040   0.9605108
## 3       Decision Tree 0.8401765 0.8359892   0.8398550
## 4       Random Forest 0.9409569 0.9384417   0.9406305
## 5                 KNN 0.9029522 0.9000549   0.9021489
\end{verbatim}
